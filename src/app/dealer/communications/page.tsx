...
export default function CommunicationsPage() {
  ...
  return (
    <div className="text-white h-[calc(100vh-64px)] flex flex-col">
      <div className="flex-1 flex flex-col max-w-7xl mx-auto w-full px-4 py-4 min-h-0">
        <h2 className="text-xl font-bold mb-3 flex-shrink-0">Communications</h2>
        <div className="flex gap-3 flex-1 min-h-0">
          {/* Left: Conversation List */}
          <div className="w-72 flex-shrink-0 bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="flex gap-1 px-2.5 py-1.5 border-b border-white/5 overflow-x-auto">
              {filters.map(f => (
            ...
            <div className="flex-1 overflow-y-auto">
              {mockConversations.map(c => (
                <button key={c.id} onClick={() => setSelectedConvo(c)} className={`w-full flex items-start gap-2.5 p-3 text-left border-b border-white/5 transition ${selectedConvo.id === c.id ? "bg-amber/10" : "hover:bg-white/[0.03]"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] text-white/20 flex-shrink-0 ml-1">{c.time}</span>
                    </div>
                  ...
                  {c.unread > 0 && <span className="w-4 h-4 rounded-full bg-amber text-navy text-[9px] font-bold flex items-center justify-center flex-shrink-0">{c.unread}</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden flex flex-col min-w-0">
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/30">Active conversation</p>
                </div>
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => { setShowCompose(!showCompose); setShowVideo(false); }}
                  className={`px-3 py-1.5 text-xs rounded-lg transition ${showCompose ? "bg-amber/20 text-amber" : "bg-white/5 text-white/40 hover:text-white"}`}
                >
                  AI Compose
                </button>
                <button
                  onClick={() => { setShowVideo(!showVideo); setShowCompose(false); }}
                  className={`px-3 py-1.5 text-xs rounded-lg transition ${showVideo ? "bg-amber/20 text-amber" : "bg-white/5 text-white/40 hover:text-white"}`}
                >
                  Video
                </button>
              </div>
            ...
            {/* Collapsible Smart Compose */}
            {showCompose && (
              <div className="border-t border-white/5 p-3 flex-shrink-0">
                <SmartCompose customerName={selectedConvo.name} vehicle="2026 Toyota RAV4" step={3} onSend={(msg) => { setNewMsg(msg); setShowCompose(false); }} />
              </div>
            )}
            ...
            {showVideo && (
              <div className="border-t border-white/5 p-3 flex-shrink-0">
                <div className="max-h-48 overflow-hidden rounded-lg">
                  <VideoRecorder onSend={(blob) => { console.log("Video recorded:", blob.size); setShowVideo(false); }} />
                </div>
              </div>
            )}
            ...
            <div className="p-3 border-t border-white/5 flex gap-2 flex-shrink-0">
              <select className="bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-xs text-white/50 flex-shrink-0">
                <option>Text</option>
                <option>Email</option>
                <option>Note</option>
              </select>
              <input value={newMsg} onChange={e => setNewMsg(e.target.value)} placeholder="Type a message..." className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 focus:border-amber outline-none min-w-0" />
              <VoiceRecorder onSave={(blob, transcript) => setNewMsg(transcript)} />
              <button className="bg-amber text-navy px-4 py-2 rounded-lg text-sm font-semibold hover:bg-amber-light transition flex-shrink-0">Send</button>
            </div>
          </div>
        </div>
      </div>
